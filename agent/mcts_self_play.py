...
def mcts_self_play(n_games, n_actions=50, model_config_path_plus=None, weight_path_plus=None,
                   model_config_path_minus=None, weight_path_minus=None, temperature=100.0, n_playout=300, c_puct=1.0, ignore_draw=False, folder = "results/bababax/kifu", n_process = 0, verbose=False):
    ...
    board_logs = []
    plus_turn_logs = []
    player_plus = MCTSPlayer(1, temperature, n_playout, c_puct, n_actions)
    player_minus = MCTSPlayer(-1, temperature, n_playout, c_puct, n_actions)
    ...
    for n in range(n_games):
        ...
        for _ in range(n_actions):
            ...
            if (verbose):
                print(gs.board)
            best_action, st, arr = player_plus.go(verbose)
            ...
            if (verbose):
                print(gs.board)
            best_action, st, arr = player_minus.go(verbose)
            if best_action is None:
                state = st
                break
            # action_logs.append(best_action)
